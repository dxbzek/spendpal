...
const Goals = () => {
  ...
  const handleAddProgress = () => {
    ...
    addGoalProgress(progressGoalId, parseFloat(progressAmount));
      ...
      <div className="px-4 -mt-4 space-y-4">
        ...
        {activeGoals.length === 0 ? (
          ...
          activeGoals.map(goal => {
            ...
            return (
              <div key={goal.id} className="bg-card rounded-2xl p-4 card-shadow">
                ...
                <div className="flex items-center justify-between text-xs text-muted-foreground mb-1">
                  <span>د.إ {fmt(goal.savedAmount)}</span>
                  <span>د.إ {fmt(goal.targetAmount)}</span>
                </div>
                <div className="h-2.5 bg-muted rounded-full overflow-hidden mb-3">
                  <motion.div initial={{ width: 0 }} animate={{ width: `${pct}%` }}
                    transition={{ duration: 0.6 }}
                    className="h-full rounded-full gradient-goals" />
                </div>
        ...
        <DialogContent className="max-w-sm mx-auto">
          ...
          <div className="space-y-4 mt-2">
            <div>
              ...
              <Input type="number" placeholder="0.00" value={progressAmount}
                onChange={e => setProgressAmount(e.target.value)} className="text-lg h-12" />
            </div>
            <Button onClick={handleAddProgress} className="w-full gradient-primary text-primary-foreground">
              Save Progress
            </Button>
          </div>
        </DialogContent>
